...
namespace gol{
    ...
    Grid::Grid(unsigned int width, unsigned int height, Color cellColor(void), bool cellAlive(unsigned int ),
               unsigned int spawn_chance) :
            width(width), height(height), size(width*height){
        grid = new Cell*[size];
        for(unsigned int x = 0; x < width; x++){
            for(unsigned int y = 0; y < height; y++){
                grid[x*height + y] = new Cell(x, y, this, cellColor(), cellAlive(spawn_chance));
            }
        }
    ...
    Cell& Grid::operator[](unsigned int x) const{
        return *grid[x];
    }
