...
int main(int argc, const char *argv[]) {
  unsigned int width = 400, height = 300, spawn_chance = 100, cell_size = 3,
               border_size = 30, frame_rate = 30;

  po::options_description desc("Usage");
  desc.add_options()("help", "produce this help message")(
      "width,w", po::value<unsigned int>(&width)->default_value(width),
      "uint, amount of cells horizontally")(
      "height,h", po::value<unsigned int>(&height)->default_value(height),
      "uint, amount of cells vertically")(
      "spawn_chance,s",
      po::value<unsigned int>(&spawn_chance)->default_value(spawn_chance),
      "uint, chance of a living cell per 1000")(
      "cell_size,c",
      po::value<unsigned int>(&cell_size)->default_value(cell_size),
      "uint, size of the individual cells in pixels")(
      "border_size,b",
      po::value<unsigned int>(&border_size)->default_value(border_size),
      "uint, size of the border in pixels")(
      "frame_rate,f",
      po::value<unsigned int>(&frame_rate)->default_value(frame_rate),
      "uint, max frame rate");
  ...
  sf::RenderWindow window(sf::VideoMode(border_size * 2 + width * cell_size,
                                        border_size * 2 + height * cell_size),
                          "Game of Life");
  ...
  for (unsigned int i = 0; i < grid.getSize(); i++) {
    ...
    cells[i].setPosition(cell_size * grid[i].getX(),
                         cell_size * grid[i].getY());
  }
  ...
  while (window.isOpen()) {
    ...
    fps.setString(std::to_string(1000 / clock.restart().asMilliseconds()) +
                  " FPS");
